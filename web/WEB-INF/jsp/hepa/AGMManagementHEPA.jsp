...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>AGM Management | HEPA</title>
    ...
    <body class="bg-light">
        ...
        <div class="main-content">
            <div class="top-header mb-4 d-flex align-items-center">
                ...
                <div>
                    <h3 class="fw-bold mb-0 text-primary"><i class="fas fa-clipboard-check me-2"></i>Final AGM Approval (HEPA)</h3>
                    <p class="text-muted mt-1 mb-0">Manage endorsements and finalize club reports.</p>
                </div>
            </div>
            ...
            <div class="card shadow-sm border-0 rounded-4">
                <div class="card-body p-4">
                    <div class="table-responsive">
                        <table id="hepaAgmTable" class="table table-hover align-middle w-100">
                            ...
                            <c:forEach var="a" items="${allReports}">
                                ...
                                <c:choose>
                                    ...
                                    <c:when test="${a.status == 'Pending_HEPA'}"><span class="badge bg-warning text-dark"><i class="fas fa-hourglass-half me-1"></i>Pending Approval</span></c:when>
                                ...
                                <c:if test="${a.status == 'Pending_HEPA'}">
                                    <form action="${pageContext.request.contextPath}/hepa/agm" method="POST" style="display:inline;">
                                        ...
                                        <button type="submit" class="btn btn-sm btn-success rounded-pill ms-1" onclick="return confirm('Confirm final approval for this club\'s AGM?')"><i class="fas fa-stamp"></i> Approve</button>
                                    </form>
                                    <button type="button" class="btn btn-sm btn-danger rounded-pill ms-1" data-bs-toggle="modal" data-bs-target="#rejectHepaModal${a.agmId}"><i class="fas fa-times"></i> Reject</button>
                                </c:if>
                                </td>
                                </tr>

                                <div class="modal fade" id="rejectHepaModal${a.agmId}" tabindex="-1">
                                    <div class="modal-dialog modal-dialog-centered"><div class="modal-content">
                                            <form action="${pageContext.request.contextPath}/hepa/agm" method="POST">
                                                <div class="modal-header bg-danger text-white"><h5 class="modal-title">Reject Approval: ${a.clubName}</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
                                                <div class="modal-body">
                                                    ...
                                                    <label class="form-label fw-bold">HEPA Rejection Remarks</label>
                                                    <textarea name="remarks" class="form-control" rows="3" placeholder="State the reason for rejection..." required></textarea>
                                                </div>
                                                <div class="modal-footer"><button type="submit" class="btn btn-danger w-100">Revoke Approval</button></div>
                                            </form>
                                        </div></div>
                                </div>
                            </c:forEach>
        ...
        <script>
                                                        $(document).ready(function () {
                                                            var table = $('#hepaAgmTable').DataTable({
                                                                "order": [[3, "desc"]], // Sort by Submitted Date (Column index 3)
                                                                "language": {
                                                                    "search": "Search:",
