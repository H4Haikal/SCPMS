...
<html lang="en">
    <head>
        <meta charset="UTF-8">
        <title>Official Review | HEPA</title>
        ...
        <style>
            ...
            .border-hepa {
                border-top: 4px solid var(--hepa-color) !important;
            }
            .bg-hepa {
        ...
        <div class="main-content">
            <div class="container-fluid py-4 px-lg-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 pb-3 border-bottom">
                    <div>
                        <a href="${pageContext.request.contextPath}/hepa/dashboard" class="btn btn-sm btn-outline-secondary mb-2 rounded-pill shadow-sm">
                            <i class="fas fa-arrow-left me-1"></i> Back to Dashboard
                        </a>
                        <h3 class="fw-bold mb-0 text-dark"><i class="fas fa-search-dollar text-hepa me-2"></i>Official Proposal Review</h3>
                    </div>
                    <a href="${pageContext.request.contextPath}/GenerateDocument?id=${p.proposalId}" class="btn btn-outline-primary rounded-pill shadow-sm px-4" target="_blank">
                        <i class="fas fa-print me-2"></i> Print / Preview PDF
                    </a>
                </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 mb-4">
                    <div class="card-body p-4 pb-2">
                        <div class="stepper-wrapper">
                            ...
                            <div class="step completed"><div class="step-icon"><i class="fas fa-check"></i></div><div class="step-label">2. Advisor</div></div>
                            <div class="step completed"><div class="step-icon"><i class="fas fa-check"></i></div><div class="step-label">3. Faculty / MPP</div></div>
                            <div class="step active"><div class="step-icon"><i class="fas fa-university"></i></div><div class="step-label">4. HEPA Final</div></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-4">
                    <div class="col-lg-8">
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-header bg-white py-3 border-0 border-bottom d-flex justify-content-between align-items-center">
                                <h5 class="fw-bold mb-0 text-hepa"><i class="fas fa-info-circle me-2"></i>Executive Summary</h5>
                            ...
                            <div class="card-body p-4">
                                ...
                                <table class="table table-borderless table-sm mb-4 bg-light rounded-3">
                                    <tbody>
                                        <tr><th class="text-muted w-25 ps-3 py-2">Organizer</th><td class="fw-bold text-dark py-2">${p.clubName}</td></tr>
                                        <tr><th class="text-muted ps-3 py-2">Category</th><td class="text-dark py-2"><span class="badge bg-secondary">${p.clubCategory}</span></td></tr>
                                        <tr><th class="text-muted ps-3 py-2">Target Date</th><td class="text-dark py-2"><i class="far fa-calendar-alt text-hepa me-2"></i> <fmt:formatDate value="${p.proposedDate}" pattern="dd MMMM yyyy" /> (${p.duration} Days)</td></tr>
                                        <tr>
                                            <th class="text-muted ps-3 py-2 pb-3">Final Request</th>
                                            <td class="text-success fw-bold py-2 pb-3 fs-5">
                                ...
                                <div class="section-title">Program Justification</div>
                                <p class="content-text">${p.description}</p>
                                <p class="content-text"><strong>Objectives:</strong><br>${p.objective}</p>
                        ...
                        <div class="card border-0 shadow-sm rounded-4 mb-4">
                            <div class="card-header bg-white py-3 border-0 border-bottom">
                                <h5 class="fw-bold mb-0 text-hepa"><i class="fas fa-list-alt me-2"></i>Operational & Financial Details</h5>
                            </div>
                            <div class="card-body p-4">
                                ...
                                <div class="table-responsive mb-4">
                                    ...
                                    <c:if test="${p.status == 'Pending_HEPA'}">
                                        <button type="button" class="btn btn-sm btn-outline-primary" data-bs-toggle="modal" data-bs-target="#alterModal">
                                            <i class="fas fa-cut me-1"></i> Make Final Budget Cuts
                                        </button>
                                    </c:if>
                                </div>
                                <div class="table-responsive mb-4">
                    ...
                    <%-- RIGHT SIDEBAR: Action Desk --%>
                    <div class="col-lg-4">
                        <div class="card border-0 shadow-lg rounded-4 border-hepa sticky-top" style="top: 80px;">
                            <div class="card-header bg-hepa text-white py-3 border-0 rounded-top-4">
                                <h5 class="fw-bold mb-0"><i class="fas fa-gavel me-2"></i>The Final Verdict</h5>
                            </div>
                            <div class="card-body p-4">
                                <c:choose>
                                    <c:when test="${p.status == 'Pending_HEPA'}">
                                        <div class="alert alert-info border-0 shadow-sm mb-4 small">
                                            <i class="fas fa-info-circle me-1"></i> This proposal has been endorsed by the Advisor and Faculty/MPP. Awaiting your final stamp.
                                        </div>

                                        <form action="${pageContext.request.contextPath}/hepa/review" method="POST" id="approvalForm">
                                            <input type="hidden" name="proposalId" value="${p.proposalId}">
                                            <input type="hidden" name="action" id="formAction" value=""> 

                                            <button type="button" class="btn btn-success w-100 rounded-pill py-3 shadow-sm fw-bold mb-3 fs-6" onclick="submitForm('approve', 'Are you sure you want to GRANT OFFICIAL APPROVAL for this proposal? The PDF will be generated immediately.')">
                                                <i class="fas fa-stamp me-2"></i> Approve & Issue PDF
                                            </button>
                                            ...
                                            <button type="button" class="btn btn-outline-danger w-100 rounded-pill py-2 fw-bold" data-bs-toggle="modal" data-bs-target="#rejectModal">
                                                <i class="fas fa-undo-alt me-2"></i> Reject & Return
                                            </button>
                                        </form>
                                    </c:when>
                                    <c:otherwise>
                                        <div class="text-center py-4">
                                            ...
                                            <h5 class="fw-bold text-dark">Decision Finalized</h5>
                                            <p class="text-muted mb-0">Status: <span class="badge bg-secondary fs-6">${p.status}</span></p>
                                        </div>
                                    </c:otherwise>
                                </c:choose>
                            </div>
        ...
        <div class="modal fade" id="rejectModal" tabindex="-1">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-4">
                    ...
                    <form action="${pageContext.request.contextPath}/hepa/review" method="POST">
                        <div class="modal-body p-4 bg-light">
                            <input type="hidden" name="proposalId" value="${p.proposalId}">
                            <input type="hidden" name="action" value="reject">
                            <div class="mb-3">
                                <label class="form-label fw-bold text-dark">Official Reason for Rejection / Amendments:</label>
                                <textarea name="rejectFeedback" class="form-control border-danger" rows="5" placeholder="State official reasons clearly for the student..." required></textarea>
                            </div>
                        </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg rounded-4">
                    <div class="modal-header bg-primary text-white border-0">
                        <h5 class="modal-title fw-bold"><i class="fas fa-cut me-2"></i>Final Budget Adjustments</h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <form action="${pageContext.request.contextPath}/hepa/review" method="POST">
                        <div class="modal-body p-4 bg-light">
                            <div class="alert alert-info border-0 small d-flex justify-content-between align-items-center">
                                <span><i class="fas fa-info-circle me-1"></i> Make final cuts before approving.</span>
                            ...
                            <div class="mb-4">
                                <label class="form-label fw-bold text-dark">Justification for Cut (Audit Trail):</label>
                                <textarea name="alterFeedback" class="form-control" rows="2" placeholder="E.g., Removed external speaker fees as per HEPA policy..." required></textarea>
                            </div>
                            ...
                            <div class="table-responsive bg-white rounded-3 border">
                                <table class="table table-hover align-middle mb-0" id="editableBudgetTable">
                                    ...
                                    <tbody>
                                        <%-- JSTL loops through current budgets! --%>
                                        <c:forEach var="b" items="${p.budgets}">
                                            <tr>
                        ...
                        <div class="modal-footer border-0">
                            <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold">Save Final Budget</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <script>
                ...
                `;
                                                    ...
                                                    document.addEventListener('DOMContentLoaded', function () {
                                                        renderSDGs();
                                                        calculateNewTotals(); // Calculate immediately on load
                                                    });
        </script>
    </body>
</html>